...
@objc class VIPSubscriptionViewController: UIViewController {
    ...
    private func setupNotifications() {
        ...
        // 监听产品加载失败/为空（尤其是模拟器未配置 StoreKit Configuration）
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(productsLoadFailed(_:)),
    ...
    @objc private func productsLoadFailed(_ notification: Notification) {
        DispatchQueue.main.async {
            let reason = (notification.userInfo?["reason"] as? String) ?? ""
            let msg = (notification.userInfo?["message"] as? String) ?? ""

            // Show a helpful hint when products can't be loaded.
            #if targetEnvironment(simulator)
            if reason == "empty" {
                self.showAlert(
                    title: "tip".localized,
                    message: "模拟器未配置 StoreKit 测试文件。请在 Xcode Scheme -> Run -> Options 里设置 StoreKit Configuration（例如 LedScroller/StoreKitConfiguration.storekit），再重试。"
                )
                return
            }
            #endif

            self.showAlert(title: "tip".localized, message: msg.isEmpty ? "loadingProducts".localized : msg)
        }
    }
    ...
    @objc private func closeTapped() {
        ...
        // Close depends on presentation style.
        // - Presented modally: dismiss.
        // - Pushed in a navigation stack: pop.
    ...
    func productsRequest(_ request: SKProductsRequest, didReceive response: SKProductsResponse) {
        DispatchQueue.main.async {
            ...
            if self.products.isEmpty {
                ...
                print("   2. Bundle ID 是否匹配: com.seventiny.ledscroller")
                print("   3. 产品ID 是否正确:")
                print("      - com.seventiny.ledscroller.vip.weekly")
                print("      - com.seventiny.ledscroller.vip.monthly")
                print("      - com.seventiny.ledscroller.vip.yearly")
